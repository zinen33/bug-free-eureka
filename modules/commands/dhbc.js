...
const coinsdown = 50000 //thay số coins bị mất khi yêu cầu gợi ý
...
module.exports.handleReply = async function ({
    ...
    case "choosee": {
        ...
        case "2": {
            ...
            const res = await axios.get(`https://raw.githubusercontent.com/hoaibaone/data/main/data.json`);
        ...
        case "1": {
            api.unsendMessage(handleReply.messageID);
            const res = await axios.get(`https://raw.githubusercontent.com/hoaibaone/data/main/data2.json`);
    ...
    case "reply": {
        ...
        if (dapan.toLowerCase() == "gợi ý" ) { 
            ...
            if (coinsdown > balance) return api.sendMessage(`Số dư không đủ ${coinsdown} Đô để xem gợi ý!!`,event.threadID,event.messageID);
            await Currencies.decreaseMoney(event.senderID, parseInt(coinsdown))
            api.sendMessage(`Gợi ý cho bạn là: \n${suggestions} (-${coinsdown} Đô)`, event.threadID, event.messageID) 
        }
        else { 
        if (dapan.toLowerCase() == tukhoa) {
            ...
            return api.sendMessage(`${name1.name} đã trả lời chính xác!\nĐáp án: ${tukhoa} (+${coinsup} BAO)`, event.threadID, event.messageID)
        } else
    ...
    case "reply2": {
        ...
        if (dapan1.toLowerCase() == "gợi ý") { 
            let balance = (await Currencies.getData(event.senderID)).money;
            if (coinsdown > balance) return api.sendMessage(`Số dư không đủ ${coinsdown} BAO để xem gợi ý!!`,event.threadID,event.messageID);
            await Currencies.decreaseMoney(event.senderID, parseInt(coinsdown))
            api.sendMessage(`Gợi ý cho bạn là: \n${suggestions} (-${coinsdown} BAO)`, event.threadID, event.messageID) 
        }
            else {
...
module.exports.run = async function ({
    ...
    if (!args[0]) {
    return api.sendMessage({body:`== [ ĐUỔI HÌNH BẮT CHỮ ] ==\n──────────────────\nVui lòng chọn chế độ chơi:\n1. Một ảnh\n2. Hai ảnh\n\n→ Vui lòng reply tin nhắn này để chọn chế độ`, attachment: (await require('axios').get(`https://i.imgur.com/WxpP7YF.jpeg`, {
        responseType: 'stream'
