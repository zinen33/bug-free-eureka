...
(async function () {
  ...
  const adminID = '100013384479798'// thay uid phây búc chúng mày vào, nếu chúng m k thấy bot nhắn về acc m thì hãy xem lại tin nhắn mày có bị mã hóa k nhá thằng lồn
  api.sendMessage('مرحبا مطوري تم تشغيل بوت ', adminID);
  //////dọn cache khi onbot!////////////////////////////////////////////////////////////
...
//DEFINE DATLICH PATH
  ...
  return async (event) => {
    ...
    let thuebot;
        ...
        if (!find_thuebot)return api.shareContact(` مرحبا , لستخدام بوت عليك اولا طلب الإذن من مطور 🥰 `, global.config.ADMINBOT[0], event.threadID);
        if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()+25200000)return api.shareContact(`Nhóm của bạn đã hết hạn sử dụng bot, vui lòng liên hệ admin để được tiếp tục gia hạn bot và sử dụng bot 😘`, global.config.ADMINBOT[0], event.threadID);
        };
