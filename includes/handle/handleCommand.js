...
if(!global.data.allThreadID.includes(threadID) && !ADMINBOT.includes(senderID) && adminbot.spadmPaseOnly == true) {
  ...
  return api.sendMessage('[ 𝗠𝗢𝗗𝗘 ] → Hiện tại đang kích hoạt chế độ vô cực cho tất cả các nhóm, chỉ ADMIN được sử dụng Bot', threadID, callback_unsend_mid, messageID)
    ...
    if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.alladmOnly == true) { 
          return api.sendMessage('[ 𝗠𝗢𝗗𝗘 ] → Hiện tại đang kích hoạt chế độ Người hỗ trợ cho tất cả các nhóm, chỉ Người hỗ trợ được sử dụng Bot', threadID, callback_unsend_mid, messageID)
    }
...
const findd = threadInf.adminIDs.find(el => el.id == senderID);
  ...
  return api.sendMessage('[ 𝗠𝗢𝗗𝗘 ] → Hiện tại đang kích hoạt chế độ Người hỗ trợ cho nhóm này, thành viên nhóm này không được sử dụng Bot', threadID, callback_unsend_mid, messageID)
    }
